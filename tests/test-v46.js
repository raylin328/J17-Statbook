...
(async () => {
  ...
  console.log("\n-- export and backups");
  {
    ...
    // read the version from the app so this doesn't break every release
    var v = A.w.eval("APP_VERSION");
    ok("version shown on Sessions", new RegExp("Stat Book " + v.replace(/\./g, "\\.")).test(A.view()));
  }
