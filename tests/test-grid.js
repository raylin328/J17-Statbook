...
ok("grid columns follow the catalog", heads.join("|") === "Player|PTS|2P|3P|1P|OREB|DREB|AST|STL|BLK|PF|TO", heads.join("|"));
...
ok("every stat is a tappable cell", A.$$(".tapgrid .cell").length === 20)   // 2 players x 10 stats (v4.7.1);
...
A.click(A.cell(0, "2P"));
A.click(A.cell(0, "1P"));
A.click(A.cell(0, "DREB"));
A.click(A.cell(0, "DREB"));
ok("mixed taps total correctly", A.$(".tscore").textContent === "6", A.$(".tscore").textContent);
ok("rebound cell counts to 2", A.cell(0, "DREB").textContent === "2");
...
A.click(A.cell(0, "DREB"));
ok("minus removes one rebound", A.cell(0, "DREB").textContent === "1");
...
ok("minus removes the newest match", A.w.eval(
  "activeSession().events.filter(e=>e.key==='DREB').map(e=>e.period).join()") === "Q1",
  A.w.eval("activeSession().events.filter(e=>e.key==='DREB').map(e=>e.period).join()"));
...
ok("grid events export to CSV", /Player 1,3,1,0,1,0,1,1/.test(csv), csv.split("\n").slice(4, 7).join(" | "));
